...
if SERVER then
	...
	function SUIT:Think()
		...
		if (!GAMEMODE:GetPlugin("Environments")) then return end
...
if SERVER then
	...
	function SUIT:HUDPaint()
		local suittimeout = SETTINGS:GetClient( "suittimeout", 30 )
		local suitposition = SETTINGS:GetClient( "suitposition", "Left Top" )

		local percent = LocalPlayer():Suit() / LocalPlayer():GetMaxSuit() * 100
		local s = utilx.Smoother(percent, "suit")
		local h = utilx.Smoother(LocalPlayer():Health(), "suithealth")

		local percent2 = 0

		if (LocalPlayer():GetMaxSuitArmor() > 0) then percent2 = LocalPlayer():SuitArmor() / LocalPlayer():GetMaxSuitArmor() * 100 end

		local s2 = utilx.Smoother(percent2, "suitarmor")
