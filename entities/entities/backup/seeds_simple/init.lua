...
function ENT:PhysicsSetup()
	local mins = self:OBBMins() * self.MaxHeight
	local maxs = self:OBBMaxs() * self.MaxHeight
...
function ENT:FireSpread()
	...
	for _, ent in pairs( ents.FindInSphere( self:GetPos(), 60 ) or {} ) do
		if (ent != self and !ent:IsOnFire() and (ent:IsLog() or ent:IsTree() or ent:IsBush())) then
			ent:Ignite()
		end
	end
end
